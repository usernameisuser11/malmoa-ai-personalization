...
export type ModeStats={generatedCount:number;selectedCount:number;selectionRate:number;averageEojeol:number;averagePersonalWordCount:number;geminiGeneratedCount:number;fallbackGeneratedCount:number};
...
export type SystemReadiness={service:string;ready:boolean;database:'ok'|'error';geminiConfigured:boolean;geminiModel:string};
export type GeminiProbe={configured:boolean;reachable:boolean;model:string;status:'ok'|'unreachable'|'not_configured'};
