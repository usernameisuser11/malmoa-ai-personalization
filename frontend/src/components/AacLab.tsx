...
import type { AacSymbol, Comparison } from '@/types';
...
export function AacLab() {
  ...
  useEffect(()=>{(async()=>{try{const [s,e]=await Promise.all([api.getSymbols(USER_ID),api.getEmergencySymbols(USER_ID)]);setSymbols(s);setEmergency(e);}catch(err){setError(err instanceof Error?err.message:'불러오기 실패');}finally{setLoading(false);}})();},[]);
  ...
  async function generate(){
    ...
    try{const result=await api.compare({userId:USER_ID,situation,intent,selectedWords:selected.map(s=>s.userAlias||s.displayText)});setComparison(result);}catch(err){setError(err instanceof Error?err.message:'AI 생성 실패');}finally{setGenerating(false);}
  }
  ...
  return <main className="page">
    ...
    {comparison&&<section className="panel"><h2>3. 추천 결과 비교</h2><p className="panel-desc">개인화 결과는 최대 어절 수와 즐겨찾기/중요/사용 빈도 어휘를 기준으로 후검증·정렬됩니다.</p><div className="two-col"><Result title="일반 AI" result={comparison.baseline} onSpeak={s=>void speak(s)}/><Result title="말모아 개인화 AI" result={comparison.personalized} personalized onSpeak={s=>void speak(s,selected.map(x=>x.userAlias||x.displayText))}/></div></section>}
  </main>;
}
